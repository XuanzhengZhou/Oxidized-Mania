...
fn rasterize_all_glyphs(extra_chars: &[char]) -> GlyphCache {
    ...
    let mut chars: Vec<char> = (32u8..=126u8)
        .map(|c| c as char)
        .chain(['★','▼','▶','◀','【','】','：','；','（','）','×','谱','面','倍','速','结','算','制','器','显','示','配','色','频','带','数','大','小','步','长','上','限','下','降','噪','门','率','游','玩','设','置','音','键','位','皮','肤','全','模','F','P','S','开','关','当','前','切','换','按','T','Y','轨','道','流','判','定','线','置','间','距','舞','台','缩','放','镜','像','偏','移','H','z','M','e','l','件','夹','录','树','展','认','择','共','首','文','目'])
        .collect();
