...
static SKIN_CACHE: std::sync::LazyLock<Mutex<HashMap<String, CachedSkin>>> =
...
fn resize_image(data: &[u8], w: u32, h: u32, target_w: u32, target_h: u32) -> (Vec<u8>, u32, u32) {
...
impl CpuSkin {
    pub fn load(skin_name: &str, cover_path: Option<&Path>) -> Self {
        inc_skin_load();
        let cache_key = format!("skin:{skin_name}:{}", cover_path.map(|p| p.to_string_lossy()).unwrap_or_default());
        if let Some(cache) = SKIN_CACHE.lock().ok() {
            if let Some(c) = cache.get(&cache_key) {
                return Self { images: c.images.clone(), config: c.config.clone() };
            }
        }
        let mut images = Vec::new();
        let mut config = HashMap::new();

        // osu! logo 最优先（缩小到高 512px，省图集空间）
        ...
        if !skin_name.is_empty() {
            ...
            if dir.exists() {
                // 先尝试 skin_config.json，再回退 skin.ini
                ...
                let mania_4k: HashMap<String, String> = if skin_config_path.exists() {
                    if let Ok(content) = std::fs::read_to_string(&skin_config_path) {
                        if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(&content) {
                            ...
                            // 尝试取 mania_configs.4 或 mania_configs."4"
                            if let Some(mc) = parsed.get("mania_configs") {
                                ...
                                if let Some(obj) = k4.and_then(|v| v.as_object()) {
                                    for (k, v) in obj {
                                        if let Some(s) = v.as_str() { cfg.insert(k.clone(), s.to_string()); }
                                    }
                                }
                            }
                            cfg
                ...
                for lane in 0..4 {
                    ...
                    // Tap note
                    let note_ref = cfg(&format!("NoteImage{}", lane), &format!("mania-note{}", ln));
                    let mut note_data = load_img(&note_ref);
                    if note_data.is_none() && lane > 0 {
                        let fallback_ref = cfg("NoteImage0", "mania-note1");
                        note_data = load_img(&fallback_ref);
                    }
                    if let Some(data) = note_data {
                        images.push((format!("note_{}", lane), data.0, data.1, data.2));
                    }

                    // Hold head
                    let head_ref = cfg(&format!("NoteImage{}H", lane), &format!("mania-note{}H", ln));
                    let mut head_data = load_img(&head_ref);
                    if head_data.is_none() {
                        head_data = images.iter().find(|(n,_,_,_)| n == &format!("note_{}", lane)).map(|(_,d,w,h)| (d.clone(), *w, *h));
                    }
                    if let Some(data) = head_data {
                        images.push((format!("hold_head_{}", lane), data.0, data.1, data.2));
                    }

                    // Hold body
                    let body_ref = cfg(&format!("NoteImage{}L", lane), &format!("mania-note{}L", ln));
                    let body_data = load_img(&body_ref).or_else(|| {
                        if lane > 0 { load_img(&cfg("NoteImage0L", "mania-note1L")) } else { None }
                    });
                    if let Some(data) = body_data {
                        images.push((format!("hold_body_{}", lane), data.0, data.1, data.2));
                    }

                    // Hold tail
                    let tail_ref = cfg(&format!("NoteImage{}T", lane), &format!("mania-note{}T", ln));
                    let mut tail_data = load_img(&tail_ref);
                    if tail_data.is_none() {
                        tail_data = images.iter().find(|(n,_,_,_)| n == &format!("hold_head_{}", lane)).map(|(_,d,w,h)| (d.clone(), *w, *h));
                    }
                    if let Some(data) = tail_data {
                        images.push((format!("hold_tail_{}", lane), data.0, data.1, data.2));
                    }

                    // Key (pressed)
                    let key_ref = cfg(&format!("KeyImage{}", lane), &format!("mania-key{}", ln));
                    if let Some(data) = load_img(&key_ref) {
                        images.push((format!("key_{}", lane), data.0, data.1, data.2));
                    }

                    // Key D (released — 通过 alpha 实现)
                    let key_d_ref = cfg(&format!("KeyImage{}D", lane), &format!("mania-key{}D", ln));
                    let key_d_data = load_img(&key_d_ref).or_else(|| {
                        images.iter().find(|(n,_,_,_)| n == &format!("key_{}", lane)).map(|(_,d,w,h)| (d.clone(), *w, *h))
                    });
                    if let Some(data) = key_d_data {
                        images.push((format!("key_{}D", lane), data.0, data.1, data.2));
                    }
                }

                // ─── 舞台 ───
                for osu_name in &[
                    cfg("StageBottom", "mania-stage-bottom"),
                    "mania-stage-bottom".to_string(),
                ] {
                    if let Some(data) = load_img(osu_name) {
                        images.push(("stage_bottom".into(), data.0, data.1, data.2));
                        break;
                    }
                }
                ...
                for (internal, cfg_key, default) in &hit_defaults {
                    let ref_name = cfg(cfg_key, default);
                    if let Some(data) = load_img(&ref_name) {
                        images.push((internal.to_string(), data.0, data.1, data.2));
                    } else {
                        // 直接尝试 default
                        if let Some(data) = load_img(default) {
                            images.push((internal.to_string(), data.0, data.1, data.2));
                        }
                    }
                }
                ...
                // 保存皮肤配置
                for (k, v) in &mania_4k { config.insert(k.clone(), v.clone()); }
                log::info!("Skin: {} images collected from {:?}", images.len(), dir);
            }
        }
        ...
        if let Some(mut cache) = SKIN_CACHE.lock().ok() {
            cache.insert(cache_key, CachedSkin { images: images.clone(), config: config.clone() });
        }
        Self { images, config }
    }
    ...
    pub fn load_menu_bg(cover_path: Option<&std::path::Path>) -> Self {
        ...
        if let Some(mut cache) = SKIN_CACHE.lock().ok() {
            cache.insert(cache_key, CachedSkin { images: images.clone(), config: HashMap::new() });
        }
        Self { images, config: HashMap::new() }
    }
...
fn build_atlas(
    ...
    const SIZE: u32 = 3072;  // 36MB — 平衡内存与容量
    ...
    TextureAtlas { texture, view, sampler, regions }
}
